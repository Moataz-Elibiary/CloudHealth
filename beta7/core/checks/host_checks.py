...
class HostHealthChecker:
    ...
    def __init__(self, nodes: List[NodeConfig], app: AppConfig,
                 ...
                 jump_host: Optional[str] = None, cluster: Optional[ClusterConfig] = None):
        self.nodes        = nodes
        self.app          = app
        self.log          = logger
        self.con          = console
        ...
        self.jump_host    = jump_host   # "user@bastion_ip" for ssh -J
    ...
    def _get_host_logger(self, ip: str) -> logging.Logger:
        """Create a dedicated rotating log file for one host node."""
        log_dir = Path(getattr(self.app, "log_dir", "~/cloud_health/logs")).expanduser()
        ...
        logger.propagate = False   # host logs stay in their own file only
        return logger
    ...
    async def _check_node_guarded(self, node: NodeConfig, sem: asyncio.Semaphore) -> Section:
        async with sem:
            return await self._check_node(node)

    # ── per-node ──────────────────────────────────────────────────────────────

    async def _check_node(self, node: NodeConfig) -> Section:
        sec = Section(f"Host: {node.ip}", "host", start_time=datetime.now())
        host_logger          = self._get_host_logger(node.ip)
        ...
        if self.con: self.con.section_done(sec)
        return sec
