...
All 19 checks run concurrently on each node for maximum speed.
"""
...
from core.inventory import NodeConfig, AppSettings, ClusterConfig, resolve_threshold
from core.models import SectionResult, Status
from core.ssh import SSHClient


class HostHealthChecker:
    """Runs all host-level diagnostics in parallel on each node."""

    def __init__(self, nodes: List[NodeConfig], app: AppSettings,
    ...
    async def run(self) -> List[SectionResult]:
        """
        Runs host diagnostics on all nodes in parallel.
        Limits concurrency using max_parallel_nodes.
        """
        ...
        for node, res in zip(self.nodes, results):
            if isinstance(res, Exception):
                s = SectionResult(f"Host: {node.ip}", "host",
    ...
    async def _check_node_guarded(self, node: NodeConfig,
                                   sem: asyncio.Semaphore) -> SectionResult:
        """Helper to run node check within a semaphore."""
        async with sem:
            return await self._check_node(node)

    # ── per-node ──────────────────────────────────────────────────────────────

    async def _check_node(self, node: NodeConfig) -> SectionResult:
        sec = SectionResult(f"Host: {node.ip}", "host",
                            start_time=datetime.now())
        if self.con:
            self.con.section_start(f"Host: {node.ip}")

        ssh = SSHClient(node.ip, node.username, node.password,
                        node.key_path, timeout=self.app.ssh_timeout)
        try:
            await ssh.connect()
    ...
    async def _r(self, ssh: SSHClient, sec: SectionResult, cmd: str,
                 timeout: int = 30):
    ...
    async def _uptime(self, ssh, sec):
        ...
        load_m = re.search(
            r"load average[s]?:\s+([\d.]+),\s*([\d.]+),\s*([\d.]+)", r.stdout)
        ...
        try:
            ...
            cpus = 1
        if load_m:
            l1  = float(load_m.group(1))
            l5  = float(load_m.group(2))
            l15 = float(load_m.group(3))
            ratio = l1 / cpus
    ...
    async def _cpu(self, ssh, sec):
        ...
        r2 = await self._r(ssh, sec,
            "nproc --all 2>/dev/null; lscpu 2>/dev/null | "
            "grep -E 'Socket|Core|Thread' | head -3")
        sec.info(f"CPU: {model} | {r2.out.strip()[:120]}")
        # Thermal throttling events
        r3 = await self._r(ssh, sec,
            "grep -r 'throttled' /sys/devices/system/cpu/cpu*/thermal_throttle/ "
        ...
        if "performance" not in r4.out and "unavailable" not in r4.out:
            sec.warn(f"CPU governor: {r4.out.strip()} "
                     "(expected 'performance' for NFV workloads)")
        elif "performance" in r4.out:
            sec.pass_("CPU governor: performance")

    # ══════════════════════════════════════════════════════════════════════════
    #  4. Memory (RAM + Swap + OOM)
    # ══════════════════════════════════════════════════════════════════════════
    async def _memory(self, ssh, sec):
        r = await self._r(ssh, sec, "free -m")
        mem_line = next(
            (l for l in r.stdout.splitlines() if l.startswith("Mem:")), None)
        if mem_line:
            ...
            if len(parts) >= 7:
                try:
                    ...
                    msg = (f"RAM: {total_mb//1024}G total, {used_mb//1024}G used "
                           f"({used_pct}%), {avail_mb//1024}G available")
        ...
        swap_line = next(
            (l for l in r.stdout.splitlines() if l.startswith("Swap:")), None)
        if swap_line:
            ...
            if len(parts) >= 3:
                try:
                    ...
                    if total_s > 0:
                        ...
                        if pct_s >= self.app.swap_used_pct_warn:
        ...
        r2 = await self._r(ssh, sec,
            ...
            "grep -c 'oom_kill\\|Out of memory' || "
            "grep -c 'Out of memory\\|oom_kill' /var/log/messages 2>/dev/null "
            "|| echo 0", timeout=20)
        try:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  5. Disk utilization (multi-mount + inode + I/O wait + SMART)
    # ══════════════════════════════════════════════════════════════════════════
    async def _disk(self, ssh, sec):
        ...
        for line in r.stdout.splitlines():
            ...
            if pct >= thr:
                sec.fail(f"Disk {parts[2]} at {pct}% (threshold: {thr}%)",
                         command="df -h")
                ok = False
        if ok:
            sec.pass_(f"All disk mounts below {thr}%",
                      detail="\n".join(detail_rows[:20]))
        # Inode usage
        r2 = await self._r(ssh, sec,
        ...
        # Disk I/O wait
        r3 = await self._r(ssh, sec,
            "iostat -x 1 2 2>/dev/null | "
            "awk '/Device/{ok=1} ok && /[a-z]/{print}' | tail -10 || true")
        if r3.out.strip():
            high_wait = []
            for l in r3.out.splitlines():
                l = l.strip()
                if not l:
                    continue
                cols = l.split()
                if len(cols) > 10:
                    try:
                        ...
                        pass
            if high_wait:
                sec.warn("High disk I/O await detected",
                         detail="\n".join(high_wait[:5]))
        # SMART status for physical drives
        r4 = await self._r(ssh, sec,
            "for d in $(lsblk -d -n -o NAME 2>/dev/null | "
            "grep -E '^sd|^nvme' | head -4); do "
            "  smartctl -H /dev/$d 2>/dev/null | "
            "  grep -E 'overall-health|PASSED|FAILED' | "
            "  awk -v d=$d '{print d\": \"$0}'; "
            "done || true", timeout=30)
        if r4.out.strip():
            failed = [l for l in r4.out.splitlines() if "FAILED" in l.upper()]
            if failed:
                sec.fail("SMART health FAILED for drive(s)",
                         detail="\n".join(failed))
            else:
                sec.pass_("SMART health: PASSED for all checked drives",
                          detail=r4.out.strip()[:300])

    # ══════════════════════════════════════════════════════════════════════════
    #  6. ECC memory errors (UE + CE + edac-util + MCE dmesg)
    # ══════════════════════════════════════════════════════════════════════════
    async def _ecc(self, ssh, sec):
        ...
        r_ce = await self._r(ssh, sec,
            ...
            "done")
        # edac-util summary
        r_edac = await self._r(ssh, sec,
            "edac-util -s 0 2>/dev/null || true")
        # dmesg MCE events
        r_mce = await self._r(ssh, sec,
            "journalctl -k --since='7 days ago' 2>/dev/null | "
            "grep -iE 'machine check|mce|corrected memory|"
            "uncorrected memory|DIMM|ECC' | tail -10 || "
            "dmesg -T 2>/dev/null | "
            "grep -iE 'machine check|mce|DIMM|ECC' | tail -10 || true",
            timeout=20)
        ...
        if r_edac.out and "error" in r_edac.out.lower():
            sec.warn("edac-util reported issues",
                     detail=r_edac.out[:200])
        # MCE dmesg
        if r_mce.out.strip():
            sec.warn("MCE/ECC messages in kernel log",
                     detail=r_mce.out.strip()[:400])
        if node_ok:
    ...
    async def _network_interfaces(self, ssh, sec):
        ...
        lines = [l for l in r.stdout.splitlines()
                 if l.strip() and "lo " not in l]
        down    = [l for l in lines if " DOWN " in l or "NO-CARRIER" in l]
        up      = [l for l in lines if " UP " in l]
        unknown = [l for l in lines
                   if " UNKNOWN " in l and "loop" not in l.lower()]
        sec.info(f"Interfaces: {len(up)} UP, {len(down)} DOWN, "
                 f"{len(unknown)} UNKNOWN")
        if down:
            unexpected = [l for l in down
                          if not re.search(r"vf|dummy|tap|veth|vxlan", l, re.I)]
            if unexpected:
                sec.warn(f"{len(unexpected)} unexpected interface(s) DOWN",
                         detail="\n".join(unexpected[:10]))
            else:
                sec.pass_("All physical interfaces UP "
                          "(virtual/VF interfaces may be DOWN as expected)")
        else:
        ...
        r2 = await self._r(ssh, sec,
            ...
            "if(rx_err+0>0||tx_err+0>0) print iface\" RX_ERR:\"rx_err"
            "\" TX_ERR:\"tx_err}' | grep -v 'lo:' | head -10 || true",
            timeout=15)
        if r2.out.strip():
            sec.warn("Interface errors detected",
                     detail=r2.out.strip()[:300])
    ...
    async def _bond_status(self, ssh, sec):
        r = await self._r(ssh, sec,
            "ls /proc/net/bonding/ 2>/dev/null || echo 'no_bonds'")
        if "no_bonds" in r.out or not r.out.strip():
            sec.info("No bond interfaces configured")
            return
        bonds = [b.strip() for b in r.out.splitlines() if b.strip()]
        for bond in bonds:
            r2 = await self._r(ssh, sec,
                f"cat /proc/net/bonding/{bond} 2>/dev/null")
            if not r2.out:
                continue
            ...
            if mii_down:
                sec.fail(
                    f"Bond {bond} ({mode}): {len(mii_down)}/{len(active)} "
                    f"slave(s) MII down", detail=r2.out[:600])
            elif total_failures > 0:
                sec.warn(
                    f"Bond {bond} ({mode}): {len(active)} slaves active, "
                    f"{total_failures} historical link failure(s)")
            else:
                sec.pass_(
                    f"Bond {bond} ({mode}): {len(active)} slave(s) all UP, "
                    f"no failures")
    ...
    async def _sriov(self, ssh, sec):
        ...
        if r.out.strip():
            ...
            sec.info(f"SR-IOV: {total_vfs} total Virtual Functions",
                     detail=r.out.strip()[:300])
            # Check VF state
            r2 = await self._r(ssh, sec,
                "ip link show 2>/dev/null | grep -A1 'vf ' | head -30 "
                "|| true")
            if r2.out.strip():
                disabled = len(re.findall(r"link-state disable", r2.out))
                if disabled:
                    sec.warn(
                        f"SR-IOV: {disabled} VF(s) with link-state disabled")
                else:
    ...
    async def _kernel_messages(self, ssh, sec):
        r = await self._r(ssh, sec,
            ...
            "grep -vE 'audit|ACPI.*Information|microcode|pcie_bw|BERT' | "
            "tail -20 || "
            "dmesg -T 2>/dev/null | "
            "grep -iE 'error|crit|emerg|alert|warn' | "
            "grep -vE 'audit|acpi|microcode' | tail -20 || true",
            timeout=25)
        lines = [l.strip() for l in r.stdout.splitlines() if l.strip()]
        critical = [l for l in lines
                    if re.search(r"emerg|alert|crit|panic|oops|BUG:", l, re.I)]
        errors = [l for l in lines
                  if re.search(r"\berr\b|\berror\b", l, re.I)]
        if critical:
            sec.fail(
                f"{len(critical)} critical kernel message(s) in last 24h",
                detail="\n".join(critical[:10]))
        elif errors:
            sec.warn(
                f"{len(errors)} kernel error message(s) in last 24h",
                detail="\n".join(errors[:10]))
        else:
    ...
    async def _systemd_services(self, ssh, sec):
        ...
        lines = [l.strip() for l in r.stdout.splitlines() if l.strip()]
        if lines:
            sec.fail(f"{len(lines)} failed systemd service(s)",
                     detail="\n".join(lines[:15]))
        else:
    ...
    async def _ntp(self, ssh, sec):
        ...
        if not r.out.strip():
            sec.warn("Cannot determine NTP status")
            return
        if re.search(
                r"synchronized.*yes|NTP service: active|System time.*offset",
                r.out, re.I):
            offset_m = re.search(
                r"System time\s*offset\s*:\s*([\d.]+)", r.out)
            if offset_m:
                offset_ms = float(offset_m.group(1)) * 1000
                if offset_ms > 100:
                    sec.warn(f"NTP offset high: {offset_ms:.1f}ms")
                else:
                    sec.pass_(f"NTP synchronized, offset: {offset_ms:.2f}ms")
            else:
                sec.pass_("NTP synchronized")
        elif re.search(
                r"synchronized.*no|NTP service: inactive", r.out, re.I):
            sec.fail("NTP not synchronized",
                     detail=r.out.strip()[:200])
        else:
    ...
    async def _pcie_errors(self, ssh, sec):
        r = await self._r(ssh, sec,
            "dmesg -T 2>/dev/null | "
            "grep -iE 'pcie.*error|aer.*error|pci.*error|nmi.*error' | "
            "grep -iv 'corrected\\|informational' | tail -10 || true")
        if r.out.strip():
            sec.warn("PCIe/AER error messages detected",
                     detail=r.out.strip()[:400])
        else:
    ...
    async def _firmware_versions(self, ssh, sec):
        r = await self._r(ssh, sec,
            ...
            "grep -E 'Vendor|Version|Release Date' | head -3 || true")
        if r.out.strip():
            sec.info(
                f"BIOS/Firmware: "
                f"{' | '.join(l.strip() for l in r.out.strip().splitlines()[:3])}")
        # NIC firmware
        r2 = await self._r(ssh, sec,
            "for d in $(ls /sys/class/net/ | "
            "grep -vE '^lo|^vir|^veth|^docker' | head -4); do "
            "  ethtool -i $d 2>/dev/null | "
            "  grep -E 'driver|firmware-version' | "
            "  awk -v if=$d '{print if\": \"$0}'; "
            "done | head -12 || true", timeout=20)
        if r2.out.strip():
            sec.info("NIC drivers/firmware",
                     detail=r2.out.strip()[:400])
    ...
    async def _hugepages(self, ssh, sec):
        ...
        if not r.out.strip():
            sec.info("Hugepages: not configured")
            return
        ...
        if total_m and free_m and size_m:
            ...
            if total > 0:
                ...
                msg = (f"Hugepages ({sz}): {used}/{total} used ({pct}%), "
                       f"{free} free")
    ...
    async def _firewall(self, ssh, sec):
        r = await self._r(ssh, sec,
            ...
            "iptables -L INPUT --line-numbers 2>/dev/null | head -5 || "
            "echo 'unknown'", timeout=10)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  19. Open ports (listening services audit)
    # ══════════════════════════════════════════════════════════════════════════
    async def _open_ports(self, ssh, sec):
        r = await self._r(ssh, sec,
            ...
            timeout=15)
        if r.out.strip():
            sec.info("Listening TCP ports",
                     detail=r.out.strip()[:500])
